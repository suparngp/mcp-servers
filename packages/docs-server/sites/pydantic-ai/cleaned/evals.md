[ Skip to content ](#pydantic-evals)
# Pydantic Evals
...
## Datasets and Cases
In Pydantic Evals, everything begins with [`Dataset`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset)s and [`Case`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Case)s:
 ...
 * **[`Case`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Case)**: A single test scenario corresponding to Task inputs, with optional expected outputs, metadata, and case-specific evaluators
...
## Evaluators
[`Evaluator`](../api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator)s analyze and score the results of your Task when tested against a Case.
...
classMyEvaluator(Evaluator):
 async defevaluate(self, ctx: EvaluatorContext[str, str]) -> float: # (2)!
...
```
 1. You can add built-in evaluators to a dataset using the [`add_evaluator`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.add_evaluator) method.
...
async defguess_city(question: str) -> str: # (3)!
 return 'Paris'
...
"""
```
 1. Create a [test case](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Case) as above
 2. Create a [`Dataset`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset) with test cases and [`evaluators`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluators)
 3. Our function to evaluate.
 4. Run the evaluation with [`evaluate_sync`](../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset.evaluate_sync), which runs the function against all test cases in the dataset, and returns an [`EvaluationReport`](../api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport) object.
 5. Print the report with [`print`](../api/pydantic_evals/reporting/#pydantic_evals.reporting.EvaluationReport.print), which shows the results of the evaluation. We have omitted duration here just to keep the printed output from changing from run to run.
_(This example is complete, it can be run "as is")_
...
## Next Steps
 ...
 2. **Understand the data model** with [Core Concepts](core-concepts/)
 3. **Explore built-in evaluators** in [Built-in Evaluators](evaluators/built-in/)
 4. **Integrate with Logfire** for visualization: [Logfire Integration](how-to/logfire-integration/)
 5. **Build comprehensive test suites** with [Dataset Management](how-to/dataset-management/)
 6. **Implement custom evaluators** for domain-specific metrics: [Custom Evaluators](evaluators/custom/)