[ Skip to content ](#dataset-management)
# Dataset Management
...
For complete details on serialization formats, JSON schema generation, and custom evaluators, see [Dataset Serialization](../dataset-serialization/).
### Save to YAML
```
fromtypingimport Any
frompydantic_evalsimport Case, Dataset
...
@dataclass
classMyCustomEvaluator(Evaluator):
 threshold: float = 0.5
...
```
For complete details on serialization with custom evaluators, see [Dataset Serialization](../dataset-serialization/).
## Generating Datasets
...
classQuestionInputs(BaseModel, use_attribute_docstrings=True): # (1)!
"""Model for question inputs."""
 question: str
"""A question to answer"""
 context: str | None = None
"""Optional context for the question"""
classAnswerOutput(BaseModel, use_attribute_docstrings=True): # (2)!
"""Model for expected answer outputs."""
 answer: str
"""The answer to the question"""
 confidence: float = Field(ge=0, le=1)
"""Confidence level (0-1)"""
classMetadataType(BaseModel, use_attribute_docstrings=True): # (3)!
...
```
 ...
 4. Call [`generate_dataset`](../../../api/pydantic_evals/generation/#pydantic_evals.generation.generate_dataset) to create a [`Dataset`](../../../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset) with 2 cases confirming to the schema.
...
"""
 ...
 """
```
 1. Generate the [`Dataset`](../../../api/pydantic_evals/dataset/#pydantic_evals.dataset.Dataset) exactly as above.
...
frompydantic_evalsimport Case, Dataset
classMyInput(TypedDict):
 query: str
 max_results: int
classMyOutput(TypedDict):
 results: list[str]
classMyMetadata(TypedDict):
 category: str
# Type-safe dataset
...
Case-specific evaluators let different cases have different evaluation criteria, which is essential for comprehensive "test coverage". Rather than trying to write one-size-fits-all evaluators, you can specify exactly what "good" looks like for each scenario. This is particularly powerful with [`LLMJudge`](../../../api/pydantic_evals/evaluators/#pydantic_evals.evaluators.LLMJudge) evaluators where you can describe nuanced requirements per case, making it easy to build and maintain golden datasets. See [Case-specific evaluators](../../evaluators/overview/#case-specific-evaluators) for detailed guidance.
...
```
## Next Steps
 * **[Dataset Serialization](../dataset-serialization/)** - In-depth guide to saving and loading datasets
 * **[Generating Datasets](#generating-datasets)** - Use LLMs to generate test cases
 * **[Examples: Simple Validation](../../examples/simple-validation/)** - Practical examples