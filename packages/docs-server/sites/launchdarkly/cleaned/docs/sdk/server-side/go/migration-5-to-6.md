...
On this page
 * [Overview](#overview)
 * [Identifying supported Go versions for the 6.0 SDK](#identifying-supported-go-versions-for-the-60-sdk)
 * [Using Go modules and import paths](#using-go-modules-and-import-paths)
 ...
 * [Changes to flag evaluation](/docs/sdk/server-side/go/migration-5-to-6#understanding-changes-to-flag-evaluation): The methods for evaluating flags now require contexts, rather than users.
 * [Create contexts, not users](/docs/sdk/server-side/go/migration-5-to-6#understanding-differences-between-users-and-contexts): Where you previously created users, now you must create contexts.
 * [Changes to attributes](/docs/sdk/server-side/go/migration-5-to-6#understanding-changes-to-built-in-and-custom-attributes): There are now fewer built-in attributes. You can still add as many custom attributes as you like, although the format has changed slightly. A flag’s targeting rules can now address fields within a JSON object.
 * [Changes to private attributes](/docs/sdk/server-side/go/migration-5-to-6#understanding-changes-to-private-attributes): You can mark specific attributes of a context as private, either across all contexts of any kind, or within a particular context or context kind.
 * [Changes to alias events](/docs/sdk/server-side/go/migration-5-to-6#understanding-changes-to-alias-events): The alias method has been removed.
...
6.0 syntax, single context with key
```
1
...
6.0 syntax, multi-context builder
```
1
| multiContext := ldcontext.NewMulti(
---|--- 
...
In version 6.0, the `SecondaryKeyAttribute` attribute has been removed. If you were previously using this attribute as part of distinguishing [percentage rollouts](/docs/home/releases/percentage-rollouts#percentage-rollout-logic), that will no longer work for your users.
...
```
To learn more, read the [API documentation](https://pkg.go.dev/github.com/launchdarkly/go-sdk-common/v3@v3.0.0/ldcontext#hdr-Designating_an_entire_attribute_as_private).
...
```
To learn more, read [`NewMulti`](https://pkg.go.dev/github.com/launchdarkly/go-sdk-common/v3@v3.0.0/ldcontext#NewMulti).
## Understanding changes to configuration options
In the 6.0 version of the SDK, several configuration options have changed:
 * The `PrivateAttributes` and `AllAttributesPrivate` configuration options both now apply to all contexts of any context kind. To learn more, read [Understanding changes to private attributes](/docs/sdk/server-side/go/migration-5-to-6#understanding-changes-to-private-attributes).
 * `PrivateAttributeNames` is now called `PrivateAttributes`. To learn more, read [Understanding changes to private attributes](/docs/sdk/server-side/go/migration-5-to-6#understanding-changes-to-private-attributes).
...
To learn more, read [`Config`](https://pkg.go.dev/github.com/launchdarkly/go-server-sdk/v6#Config).
