...
## Overview
This topic explains the changes in the Python SDK 8.0 release and how to adapt code that uses a 7.x version of the [Python SDK](/docs/sdk/server-side/python) to use version 8.0 or later.
 ...
 * [Changes to flag evaluation](/docs/sdk/server-side/python/migration-7-to-8#understanding-changes-to-flag-evaluation): The methods for evaluating flags now require contexts, rather than users.
 * [Create contexts, not users](/docs/sdk/server-side/python/migration-7-to-8#understanding-differences-between-users-and-contexts): Where you previously created users, now you must create contexts.
 * [Changes to attributes](/docs/sdk/server-side/python/migration-7-to-8#understanding-changes-to-built-in-and-custom-attributes): There are now fewer built-in attributes. You can still add as many custom attributes as you like, although the format has changed slightly. A flag’s targeting rules can now address fields within a JSON object.
 * [Changes to private attributes](/docs/sdk/server-side/python/migration-7-to-8#understanding-changes-to-private-attributes): You can mark specific attributes of a context as private, either across all contexts of any kind, or within a particular context or context kind.
 * [Changes to alias events](/docs/sdk/server-side/python/migration-7-to-8#understanding-changes-to-alias-events): The alias method has been removed.
...
The methods for [evaluating flags](/docs/sdk/features/evaluating) and determining [flag evaluation reasons](/docs/sdk/features/evaluation-reasons) have changed slightly. The 8.0 version of the SDK includes the following changes:
...
8.0 syntax, single context shortcut
```
1
...
You can define additional attributes for a context by passing in a name and value for each. Additional attributes can be any JSON type, including boolean, number, string, array, or object. In version 8.0, you do not need to add custom attributes within a `custom` object.
...
7.x syntax, attribute marked private for one user8.0 syntax, attribute marked private for one context
```
1
| user = {
---|--- 
...
For a full list of deprecated modules, read the [Deprecated modules](https://launchdarkly-python-sdk.readthedocs.io/en/7.6.1/api-deprecated.html) section of the 7.x SDK API documentation.
