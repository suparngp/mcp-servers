...
## Overview
This topic summarizes changes and explains how to adapt code that uses a 6.x version of the [Node.js (server-side) SDK](/docs/sdk/server-side/node-js) to use version 7.0 or later.
 ...
 * [Changes to flag evaluation](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-flag-evaluation): The methods for evaluating flags now require contexts, rather than users.
 * [Create contexts, not users](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-differences-between-users-and-contexts): Where you previously created users, now you must create contexts.
 * [Changes to attributes](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-built-in-and-custom-attributes): There are now fewer built-in attributes. You can still add as many custom attributes as you like, although the format has changed slightly. A flag’s targeting rules can now address fields within a JSON object.
 * [Changes to private attributes](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-private-attributes): You can mark specific attributes of a context as private, either across all contexts of any kind, or within a particular context or context kind.
 * [Changes to alias events](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-alias-events): The alias method has been removed.
...
6.x syntax, user with key7.0 syntax, context with key, JavaScript7.0 syntax, context with key, TypeScript
```
1
| const user = {
---|--- 
...
Overall, this should make your upgrade easier, because your existing code will continue to work, as long as you don’t make changes to your flag configuration or [bucket users based on the “secondary” attribute](/docs/sdk/server-side/node-js/migration-6-to-7#removing-the-secondary-attribute).
However, if you are using version 7.0 of the SDK and you are omitting the `kind` attribute, then the following caveats apply:
 * The fields in your user object must be `LDUser` fields, not `LDContext` fields. For example, to mark an attribute as private, you must use `privateAttributeNames` in the user object, not `_meta.privateAttributes` as you would for a context object. Nested JSON, such as is used in `_meta.privateAttributes`, is not supported by older versions of the SDK. To learn more, read [Understanding changes to private attributes](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-private-attributes).
 * Any additional attributes in your user object need to be inside the `custom` property of the `LDUser`, not at the top-level as they would in a context object. To learn more, read [Working with built-in and custom attributes](/docs/sdk/server-side/node-js/migration-6-to-7#working-with-built-in-and-custom-attributes).
...
7.0 syntax, JavaScript7.0 syntax, TypeScript
```
1
| const context = {
---|--- 
...
6.x syntax, user with attributes7.0 syntax, JavaScript: context with attributes7.0 syntax, Typescript: context with attributes
```
1
| const user = {
---|--- 
...
In version 7.0, the `secondary` attribute has been removed. If you were previously using this attribute as part of distinguishing [percentage rollouts](/docs/home/releases/percentage-rollouts#percentage-rollout-logic), that will no longer work for your users.
...
```
To learn more, read the SDK’s API documentation for [`allAttributesPrivate`](https://launchdarkly.github.io/node-server-sdk/interfaces/_launchdarkly_node_server_sdk_.LDOptions.html#allAttributesPrivate) and [`privateAttributes`](https://launchdarkly.github.io/node-server-sdk/interfaces/_launchdarkly_node_server_sdk_.LDOptions.html#privateAttributes).
 ...
 * The `privateAttributes` and `allAttributesPrivate` configuration options both now apply to all contexts of any context kind. To learn more, read [Understanding changes to private attributes](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-private-attributes).
 * `privateAttributeNames` is now called `privateAttributes`. To learn more, read [Understanding changes to private attributes](/docs/sdk/server-side/node-js/migration-6-to-7#understanding-changes-to-private-attributes).
