...
On this page
 * [Overview](#overview)
 * [Using the Relay Proxy’s status resource](#using-the-relay-proxys-status-resource)
 * [Validating your Relay Proxy configuration](#validating-your-relay-proxy-configuration)
 * [Status requests](#status-requests)
 * [Testing feature flag evaluations](#testing-feature-flag-evaluations)
 * [Testing Relay Proxy performance](#testing-relay-proxy-performance)
 * [Monitoring Relay Proxy performance](#monitoring-relay-proxy-performance)
 * [Monitoring the Relay Proxy with a data store](#monitoring-the-relay-proxy-with-a-data-store)
 * [Exporting metrics and traces](#exporting-metrics-and-traces)
## Overview
...
The Relay Proxy exposes a status resource containing details about its health. To learn about the status resource, read the Relay Proxy GitHub repository’s [Status (health check)](https://github.com/launchdarkly/ld-relay/blob/v8/docs/endpoints.md#status-health-check).
...
---|--- 
```
To view an example output for this request, read the Relay Proxy GitHub repository’s [Service endpoints](https://github.com/launchdarkly/ld-relay/blob/v8/docs/endpoints.md).
...
Here’s how:
Evaluate a feature flag, Relay Proxy v7.0+
```
$
...
```
To learn more, read [Troubleshooting SDK connection to Relay](https://support.launchdarkly.com/hc/en-us/articles/18239626123547-Troubleshooting-SDK-connection-to-Relay).
...
Performance is always dependent on specific workload and environment. You can run benchmarks against your own infrastructure and LaunchDarkly configuration for more accurate information. We recommend that you configure the Relay Proxy to be highly available, which reduces load on any particular server. To learn more, read [Relay Proxy guidelines](/docs/sdk/relay-proxy/guidelines).
...
To learn more about configuring the Relay Proxy to export metrics and traces, read the Relay Proxy GitHub repository’s [Metrics integrations](https://github.com/launchdarkly/ld-relay/blob/v8/docs/metrics.md).
