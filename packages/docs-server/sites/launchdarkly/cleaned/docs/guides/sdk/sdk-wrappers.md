 ...
 * [Understanding why to wrap an SDK](#understanding-why-to-wrap-an-sdk)
 * [Using SDK wrappers in different design patterns](#using-sdk-wrappers-in-different-design-patterns)
 * [Bridge patterns](#bridge-patterns)
 * [Composite patterns](#composite-patterns)
 * [Mid-session update policies (client-side)](#mid-session-update-policies-client-side)
 * [Encapsulated SDKs](#encapsulated-sdks)
 * [Use cases for SDK wrappers](#use-cases-for-sdk-wrappers)
 * [Migrating from an existing flag system](#migrating-from-an-existing-flag-system)
 * [Acting as a mock for tests](#acting-as-a-mock-for-tests)
 * [Automatically providing context](#automatically-providing-context)
 * [Enforcing correct definition of the context](#enforcing-correct-definition-of-the-context)
 * [Improving observability](#improving-observability)
 * [Providing a single source of fallback flag values](#providing-a-single-source-of-fallback-flag-values)
 * [Acting as an evaluation error handler](#acting-as-an-evaluation-error-handler)
 * [Combining the SDK & API client behind one interface](#combining-the-sdk--api-client-behind-one-interface)
...
More use cases are explained in [Use cases for SDK wrappers](/docs/guides/sdk/sdk-wrappers#use-cases-for-sdk-wrappers).
## Prerequisites
...
##### Calling the correct methods for singletons
...
You can also encapsulate an `LDClient` instance with a context so you can pass them as a single object. This is the existing pattern provided by our client-side SDKs. It’s also useful for server-side SDKs in apps that have to evaluate multiple flags for the same context as part of a request or session. In this use case, the object also acts as a single request/session context, which is useful for observability purposes.
...
To learn more, read [Migrating your existing feature flag solution to LaunchDarkly](/docs/guides/account/migrating-solutions).
...
You can use an SDK wrapper to automatically add environmental context, such as operating system (OS), browser, or app version. A wrapper can also read user context from environment variables. To learn more about setting up different context kinds, read [Context kinds](/docs/home/flags/context-kinds).
...
For example, an SDK wrapper can use the `EvaluationReason` to handle situations where the code needs to be sure that a `false` value is coming from the flag evaluation, and not a fallback value served due to lack of connection to or data from LaunchDarkly.
