 ...
 * [Create the Kinesis Destination in LaunchDarkly](#create-the-kinesis-destination-in-launchdarkly)
 * [Test a Kinesis Destination](#test-a-kinesis-destination)
 * [Consume the Kinesis Data Stream](#consume-the-kinesis-data-stream)
 * [Enable Data Export for flags and environments](#enable-data-export-for-flags-and-environments)
 * [Delete a Kinesis destination](#delete-a-kinesis-destination)
## Overview
...
| }
```
 1. If you have a _stream ARN_ already configured, you can paste the entire ARN in. At least, change the following fields to include the values you specified during IAM policy configuration:
 * `{region}`
 * `{account-id}`
 * `{stream-name}`.
 1. Click **Save**.
...
![The Account ID field in the "Create role" screen.](https://files.buildwithfern.com/https://launchdarkly.docs.buildwithfern.com/docs/384409a6e7a43bb8f97585e485e4b57d42205011abade3791276fc92e1cb99f7/assets/images/__third_party/kinesis-create-role.png)
...
## Create the Kinesis Destination in LaunchDarkly
...
To create the Kinesis destination:
 ...
 2. Click **Integrations** and find “Amazon Kinesis.”
 ...
 6. Enter the AWS data you saved earlier:
 * AWS Kinesis stream region
 * Role address
 * AWS Kinesis stream name
 1. Click **Save destination**.
## Test a Kinesis Destination
...
##### Partition Keys
Kinesis records have [partition keys](https://docs.aws.amazon.com/streams/latest/dev/key-concepts.html#partition-key). When LaunchDarkly creates a record, it creates a random partition key.
## Consume the Kinesis Data Stream
