...
On this page
 * [Overview](#overview)
 * [Create migration flags](#create-migration-flags)
 * [Modify the migration flag template](#modify-the-migration-flag-template)
## Overview
...
Migration flags can have two, four, or six stages. To learn more about when to use different types of migrations, read [Performing multi-stage migrations with migration flags](/docs/guides/flags/migrations).
##### Not all SDKs support migration flags
Not all SDKs are compatible with migration flags. For a full list of compatible SDKs, read [supported SDKs](/docs/home/flags/migration#supported-sdks).
...
Here’s how to create a migration flag:
 ...
 5. (Optional) Click **No template** and select **2-** , **4-** , or **6-stage migration**.
 * If you selected 6-stage migration, select a **Context kind**.
...
You must choose a context kind to create a six-stage migration. The context kind you choose should indicate how the migrated data is partitioned. It determines how rollouts are bucketed and which context kind to reference when you create cohorts.
 1. (Optional) Click on your member name to change the flag maintainer.
 2. (Optional) Click on the tag icon to add one or more [tags](/docs/home/account/tags).
 3. Click **Create flag**.
The flag **Targeting** tab appears.
This procedure explains how to create a migration flag, but targeting and rolling them out is more complex. We recommend you also read [Targeting with migration flags](/docs/home/flags/target).
You can make further changes to it as needed, such as:
