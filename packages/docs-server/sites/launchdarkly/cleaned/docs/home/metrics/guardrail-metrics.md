 ...
 * [How guardrail metrics work](#how-guardrail-metrics-work)
 * [Auto-attachment behavior](#auto-attachment-behavior)
 * [Limitations](#limitations)
 * [Example](#example)
## Overview
This topic explains how to use guardrail metrics to provide a consistent set of default metrics for evaluating guarded rollouts.
Guardrail metrics can help organizations standardize how they measure the safety and success of feature releases. By defining a trusted set of metrics in a dedicated metric group, admins can ensure that every guarded rollout starts with a consistent baseline for measurement. This reduces setup time, promotes standard practices, and helps teams move faster with confidence.
You can also add guardrail metrics as secondary metrics in experiments.
## Prerequisites
To create or update guardrail metrics, members must have one of the following:
 * An Admin or Owner base role in your LaunchDarkly account, or a role with [guardrail metrics group permissions](/docs/home/account/roles/role-actions#guardrail-metrics-group-actions)
 * At least one [custom metric](/docs/home/metrics/create-metrics#choose-a-metric-type) defined in the project
 * A consistent [randomization unit](/docs/home/releases/context-kinds#randomization-units), such as `user` or `account`, across the metrics you want to include
## How guardrail metrics work
To begin, Admins, Owners, or other members with proper permissions define guardrail metrics for a project. Then, when any member creates a new [guarded rollout](/docs/home/releases/guarded-rollouts), LaunchDarkly displays “Guardrail metrics” at the top of the metric group selector and includes them automatically.
...
You can manually remove a guardrail metrics group from a rollout if needed.
### Limitations
Guardrail metrics groups include the following limitations:
 * Each project can define only one guardrail metrics group
 * All metrics in the group must use the same randomization unit, such as `user` or `account`
