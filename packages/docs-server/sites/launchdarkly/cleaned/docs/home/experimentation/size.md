 ...
 * [The exposures graph](#the-exposures-graph)
## Overview
...
Before you start a frequentist experiment, you can use LaunchDarkly’s [sample size calculator](https://launchdarkly.com/sample-size-calculator) to estimate the minimum detectable effect and sample size and duration for your experiments.
...
![An experiment's sample size estimator results.](https://files.buildwithfern.com/https://launchdarkly.docs.buildwithfern.com/docs/61c5a1f144d0bfbb3464128b170e06914237c536f044368c3eec93ac91b927ae/assets/images/__LD_UI_no_test/experiment-sample-size-estimator.png)
 ...
 * The table only counts contexts with the same context kind as the experiment’s randomization unit. For example, if your experiment’s randomization unit is `user`, then the table counts only `user` contexts. The table won’t include any `device` or `organization` contexts that are in the experiment. To learn more, read [Randomization units](/docs/home/experimentation/randomization).
